...
output_date = datetime.datetime.now().strftime("%Y-%m-%dT%H:%M:%S.%f%z")
val = "YYYY-MM-DD[T]HH:mm:ss"
val = "2022-10-06T10:51:34.765Z"
print(str(output_date))
# Get the data from the API
st.write(output_date)
data = req.get("https://api.data.gov.sg/v1/transport/carpark-availability?date_time=" + str(output_date) ).json()
